// ==UserScript==
// @name:en      Linux.do LARGE Like Counter
// @name         Linux.do 大型点赞计数器
...
(function() {
    ...
    const CONFIG = {
        ...
        SCALE_FACTOR: GM_getValue('linuxdo_reaction_scale', 3)
    };

    // --- Tampermonkey Menu for Scale Setting ---
    const SCALE_STORAGE_KEY = 'linuxdo_reaction_scale';
    const currentScale = CONFIG.SCALE_FACTOR;

    function setScale(newScale) {
        GM_setValue(SCALE_STORAGE_KEY, newScale);
        location.reload();
    }

    GM_registerMenuCommand(`⚙️ 当前缩放: ${currentScale}x`, () => {
        const input = prompt(`输入缩放倍数 (0.5 - 5):\n当前: ${currentScale}`, currentScale);
        if (input !== null) {
            const val = parseFloat(input);
            if (!isNaN(val) && val >= 0.5 && val <= 5) {
                setScale(val);
            } else {
                alert('请输入 0.5 到 5 之间的数字');
            }
        }
    });
    ...
    const btnMargin = 5 * scale;
    const btnPadding = 2 * scale;
    const containerPadding = 4 * scale;
    ...
    GM_addStyle(`
        ...
        .discourse-reactions-picker .discourse-reactions-picker-container { margin-top: 0 !important; border-top-left-radius: 0 !important; border-top-right-radius: 0 !important; padding: ${containerPadding}px ${containerPadding * 1.5}px !important; }
